...
%		\end{bmatrix}
%\end{equation}
%Virheet minimoituvat, kun näille matriiseille pätee
%\begin{equation} \label{matriisit}
%	\matr{Y} = \matr{X}\matr{\beta}.
%\end{equation}
%Tästä voidaan edelleen ratkaista $\matr{\beta}$, josta saadaan parhaiten dataan sopivan käyrän funktion kertoimet kertomalla ensin vasemmalta matriisin $\matr{X}$ transpoosilla $\transpose{\matr{X}}$, jolloin yhtälö \ref{matriisit} saadaan muotoon
%\begin{equation*}
%	\transpose{\matr{X}}\matr{Y} = \transpose{\matr{X}}\matr{X}\matr{\beta}.
%\end{equation*}
%Mikäli matriisi $\transpose{\matr{X}}\matr{X}\matr{\beta}$ on kääntyvä, voidaan nyt kertoa vasemmalta tämän matriisin käänteismatriisilla, jolloin saadaan ratkaistuksi kertoimet sisältävälle vektorille $\matr{\beta}$ yhtälössä \ref{beta} esitetty lauseke.~\cite{wolframmath}
%\begin{equation}\label{beta}
%	\matr{\beta} = (\transpose{\matr{X}}\matr{X})^{-1}\matr{X}\matr{Y}
%\end{equation}
